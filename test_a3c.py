...
class ActorCritic(nn.Module):
    ...
    def calc_returns(self, done):
        ...
        G = v[-1]*(1-int(done)) #enten så har vi flere steg, eller så blir den 0
        batch_return = []
        for reward in self.rewards[::-1]:
            G = reward + self.gamma*G
            batch_return.append(G)
    ...
    def calc_loss(self, done):
        states = T.tensor(self.states, dtype = T.float)
        action = T.tensor(self.actions, dtype = T.float)
...
if __name__ == '__main__':
    ...
    n_actions = 3
    input_dims = [4]
    N_GAMES = 10 #disse har jeg hardkodet i klassen, obsobs
    T_MAX = 5#samme
